...
This file is an architecture decision record for what was deliberately **not** built, and why. Read it if you want to know why a feature you might expect (a web UI, Docker, an agent framework) is absent, rather than assume it was simply skipped for lack of time.

Each entry is a considered rejection, not an apology. The reasoning should be evaluable on its own, independent of the deadline that also motivated it.
...
**What we did instead:** `report.py` renders one self-contained static HTML file via jinja2 (a Python templating library); every byte, including CSS, is inlined. No server, no JS framework, no build step. Opens directly in a browser from the repo.
...
**What we did instead:** `generate.py`'s seeded synthetic data, plus a narration-noise generator (project milestone E10) built specifically because narration realism, not transaction realism, was the actual open question. It was addressed directly rather than by hoping a test-mode API's own narrations would happen to be messy in the right way.
...
**Would revisit if:** the first loop were fully closed and de-risked (it isn't quite; see ARCHITECTURE.md's operating envelope section) and there were time earmarked specifically for breadth rather than depth.
...
**Rejected because:** this problem is exact, at every stage. A UTR (Unique Transaction Reference, the bank's own reference number for a transfer) either appears in a narration or it doesn't (L0), an amount ties to a batch net exactly or within a fee-derived tolerance or it doesn't (L1/L3), a subset of real settlement lines sums to the credit exactly or it doesn't (L2). None of that is a similarity-search problem. Even rapidfuzz-level string similarity was considered and explicitly rejected for L0 (project milestone E10c) as more power than the measured problem needed; embeddings would add a whole model class, an index, and a threshold to tune on top of that, for answers that are already checkable.
...
**Rejected because:** the same measurement found pools sitting *just under* the current cap already cost several seconds each (meet-in-the-middle, the technique that splits the candidate pool in half and searches each half separately, has a ~2^24-per-half worst case here). Raising the cap relocates that expensive zone to a higher line count without removing it, and pushes the true worst case further into hang territory. "Refusing to solve is correct behaviour; hanging is not" is `compose.py`'s own stated principle. Raising the cap specifically to shrink an exception count is tuning the system to make a number look better, which METRICS.md's honesty rules exist to forbid.
...
**Would revisit if:** the underlying fee structure changed, e.g. a new fee tier or a different GST treatment. The band should track the fees it exists to absorb, never the metrics it produces.
...
**Considered because:** on the adversarial dataset every `duplicate_utr` credit that reached L4 (the LLM adjudication stage) was classified as `ambiguous_composition` instead: a consistent, 0%-accuracy confusion, not scattered noise.
...
**What we did instead:** one retry, then degrade to `adjudication_failed`, a real, visible exception row, never a silent drop.

**Would revisit if:** a real prompt/model combination showed a nonzero, non-negligible malformed rate where a second correction demonstrably helped.

**2026-09-01 update (see FAILURES.md):** that evidence has now arrived, and the reasoning above was tested directly rather than left open. Schema-level malformed JSON is still 0 across every live call measured in this project, confirming the strict schema enforcement at the API boundary is doing its job. But a second check, added the same day, validates the response's content against the prompt's own facts, not just its shape (`evidence_refs` must name a record the model was actually shown). That check rejects the large majority of responses across every seed and both LLM-touching arms, and retrying with the validation error appended mostly does not recover a valid response: 45 of the with-LLM arm's 69 credits needed a retry across all six seeds, and only 11 of those 45 retries recovered a valid response; the other 34 failed again and degraded to `adjudication_failed`, 2 to 8 credits per seed where none did before. This is the second branch of the original reasoning, confirmed rather than assumed: for this failure mode, the model is hitting something a single correction usually cannot fix, and a bounded number of retries is the correct response to that, not a workaround for a rare flake. A second retry was not tried; nothing measured here suggests it would recover credits the first retry didn't.
...
**Rejected because:** this is a real improvement made in the wrong week. It touches every decision write in every stage, `audit.py`'s schema, `metrics.py`'s scoring, and `report.py`'s rendering all at once, for a benefit (structured querying over reasons) nothing in this submission's judging criteria currently needs. `unbatch exceptions` and the HTML report already read reason strings as the unit of explanation; nothing downstream is blocked on them being unstructured.

**What we did instead:** kept `reason` as a fixed enum-like string per stage (each stage has exactly one constant reason value, e.g. `l3_tolerance.REASON_MATCH`) and `rationale` as free text for L4's LLM explanation. That is structured enough for the report and exception query to group by, without a new schema migration.
