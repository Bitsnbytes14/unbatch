...
## Operating envelope (E11c, updated E12, E13, E14, and a post-E14 ablation-pooling pass)
...
**The ablation axis, pooled:** the three-arm ablation (rules-only vs. rules+LLM vs. LLM-only) was originally measured on seed 42 alone — 88.6% / 88.6% / 13.3% match rate, 0.0% false-match rate on all three arms — while everything around it was already pooled across six seeds, exactly the "one cherry-picked match" shape the brief warns against. `bench_ablation_seeds.json` extends arms B and C across all six seeds (arm A was already pooled in bench_multiseed.json); this needed live calls for arm C on seeds 43-47 (arm B's cache already covered all six from E13), reported below. Pooled across 630 credits: arm B holds at **89.2% match / 0.89% false-match** — indistinguishable from arm A's own pooled 89.05% / 0.89%, confirming the near-zero B−A delta is not a seed-42 artifact. Arm C is a **fourth instance of the headline being seed-specific**, and this time in the direction that matters most: pooled match rate is **9.5% (60/630)**, *worse* than seed 42's standalone 13.3% (per-seed range 5.7%-13.3%), and pooled false-match rate is **1.67%, not 0.0%** — driven entirely by seed 46, where LLM-only produces a real false match at a 7.1% rate (1 of 14 resolved), the only nonzero result among the six seeds. Seed 42 alone made LLM-only look merely worse and cost more; pooled, it is also measurably *less safe* — a claim the single-seed table never made and never could have.
...
**In one sentence:** this cascade is reliable against messy *text* at any measured scale and independently of how collision-prone the underlying data is; it is unreliable, to a small but non-zero and now-quantified degree, against data where independently-generated amounts happen to collide — a risk that triples to quintuples under deliberately hostile construction; the ablation itself holds up pooled (89.2% vs. seed 42's 88.6%) while the LLM-only counter-experiment turns out worse *and* less safe pooled than it looked on one seed (9.5% match, 1.67% false-match vs. 13.3%/0.0%); the model's break-reason classification, pooled across six seeds rather than trusted from one, holds up close to its original headline on organic data (89.9% vs. 91.7%, n=69) but degrades sharply and systematically (37.5%, mostly one consistent confusion) on hostile data; and the forward cash forecast, honestly backtested rather than shipped on faith, captures only the ~10% of future inflow that a short settlement lag makes visible at all — five numbers, not the single cleanest one, are the honest reason the exception list, not the match-rate headline, is this project's real deliverable.
