...
**The ablation axis, pooled:** the three-arm ablation (rules-only vs. rules+LLM vs. LLM-only) was originally measured on seed 42 alone: 88.6% / 88.6% / 13.3% match rate, 0.0% false-match rate on all three arms, while everything around it was already pooled across six seeds, exactly the "one cherry-picked match" shape the brief warns against. `bench_ablation_seeds.json` extends arms B and C across all six seeds (arm A was already pooled in bench_multiseed.json); this needed live calls for arm C on seeds 43-47 (arm B's cache already covered all six from milestone E13), reported below.
...
**2026-09-01 update (see FAILURES.md):** a semantic-validation check added to the adjudicator (evidence_refs must reference a record the model was actually shown, not an unrelated identifier such as a settlement UTR it read off a candidate hint) rejected most of the responses behind the numbers above, since citing an out-of-scope identifier turned out to be the model's default habit on this prompt, not an occasional slip. Retrying with the validation error appended mostly did not recover a classification: across all six seeds, only 35 of the original 69 credits (now with retries filled in from live calls) still receive a real classification, the rest degrading to `adjudication_failed`. Pooled accuracy on that smaller, now-correct set is 85.7% (30 of 35). Seed 42 alone moved from 91.7% (11 of 12) to 83.3% (5 of 6), with 6 of its 12 L4 credits now ending in `adjudication_failed` where none did before. The pooled confusion table on the current data: `ambiguous_composition` is classified reliably (18 of 20 where classified), `tolerance_ambiguous` similarly (9 of 11), `unrelated_credit` perfectly (3 of 3), and the single `duplicate_utr` instance that reached a classification was misclassified. On the adversarial dataset (also run for real, also cached), only 3 of 8 credits still classify at all after the same fix, and none of the 3 are correct: accuracy is 0.0%, down from 37.5% (3 of 8) before the fix. The confusion is consistent with the earlier finding rather than new: `duplicate_utr` credits are still called `ambiguous_composition`.
